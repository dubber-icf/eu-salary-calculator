...
export async function GET(request: NextRequest) {
  ...
  if (id) {
    const result = await query('SELECT * FROM staff WHERE id = $1', [parseInt(id)]);
    if (result.length === 0) {
...
export async function PUT(request: NextRequest) {
  ...
  const existing = await query('SELECT * FROM staff WHERE id = $1', [parseInt(id)]);
...
export async function DELETE(request: NextRequest) {
  ...
  await query('DELETE FROM monthly_entries WHERE staff_id = $1', [parseInt(id)]);
  await query('DELETE FROM payments WHERE staff_id = $1', [parseInt(id)]);
  await query('DELETE FROM staff WHERE id = $1', [parseInt(id)]);
